...
class GameServer {
    ...
    suspend fun memberJoin(memberId: String, socket: WebSocketSession) {
        ...
        val list = members.computeIfAbsent(memberId) { CopyOnWriteArrayList<WebSocketSession>() }
        list.add(socket)
    ...
    suspend fun broadcastCommand(command: String, room: String, sender: String, message: String) {
        val massageData = "@$room#$sender->/$command:$message"
        println("Broadcast Game Command: $massageData")
        broadcast(massageData)
    }
